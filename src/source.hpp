...
private:
	// Server-side handle. Invalid when the source is not currently registered.
	godot::RID rid;
	...
	// Registers/unregisters this source's RID with the server and pushes config.
	void register_on_server();
	void unregister_from_server();
	// Pushes the current property values to the server (no-op if not registered).
	void push_config();

	// Static RID → node lookup so RID-based results (e.g. listener sensor slots)
	// can be resolved back to a node without the server knowing about nodes.
	static godot::HashMap<int64_t, SteamAudioSource *> rid_to_node;
...
public:
	...
	static SteamAudioSource *for_rid(godot::RID p_rid);
	...
	void set_direct_enabled(bool p_enabled) { direct_enabled = p_enabled; push_config(); }

	bool get_binaural_enabled() const { return binaural_enabled; }
	void set_binaural_enabled(bool p_enabled) { binaural_enabled = p_enabled; push_config(); }
	int get_binaural_interpolation() const { return binaural_interpolation; }
	void set_binaural_interpolation(int p_interpolation) { binaural_interpolation = p_interpolation; push_config(); }

	bool get_reflection_enabled() const { return reflection_enabled; }
	void set_reflection_enabled(bool p_enabled) { reflection_enabled = p_enabled; push_config(); }
	float get_reflection_duration() const { return reflection_duration; }
	void set_reflection_duration(float p_duration) { reflection_duration = p_duration; push_config(); }
	float get_reflection_hybrid_delay() const { return reflection_hybrid_delay; }
	void set_reflection_hybrid_delay(float p_delay) { reflection_hybrid_delay = p_delay; push_config(); }

	bool get_occlusion_enabled() const { return occlusion_enabled; }
	void set_occlusion_enabled(bool p_enabled) { occlusion_enabled = p_enabled; push_config(); }
	int get_occlusion_type() const { return occlusion_type; }
	void set_occlusion_type(int p_type) { occlusion_type = p_type; push_config(); }
	float get_occlusion_radius() const { return occlusion_radius; }
	void set_occlusion_radius(float p_radius) { occlusion_radius = p_radius; push_config(); }
	int get_occlusion_samples() const { return occlusion_samples; }
	void set_occlusion_samples(int p_samples) { occlusion_samples = p_samples; push_config(); }

	bool get_transmission_enabled() const { return transmission_enabled; }
	void set_transmission_enabled(bool p_enabled) { transmission_enabled = p_enabled; push_config(); }
	int get_transmission_type() const { return transmission_type; }
	void set_transmission_type(int p_type) { transmission_type = p_type; push_config(); }
	int get_transmission_rays() const { return transmission_rays; }
	void set_transmission_rays(int p_rays) { transmission_rays = p_rays; push_config(); }

	bool get_air_absorption_enabled() const { return air_absorption_enabled; }
	void set_air_absorption_enabled(bool p_enabled) { air_absorption_enabled = p_enabled; push_config(); }

	bool get_distance_attenuation_enabled() const { return distance_attenuation_enabled; }
	void set_distance_attenuation_enabled(bool p_enabled) { distance_attenuation_enabled = p_enabled; push_config(); }
	float get_distance_attenuation_min() const { return distance_attenuation_min; }
	void set_distance_attenuation_min(float p_min) { distance_attenuation_min = p_min; push_config(); }
	float get_distance_attenuation_max() const { return distance_attenuation_max; }
	void set_distance_attenuation_max(float p_max) { distance_attenuation_max = p_max; push_config(); }

	uint32_t get_layers() const { return layers; }
	void set_layers(uint32_t p_layers) { layers = p_layers; push_config(); }
	...
	void set_doppler_factor(float p_factor) { doppler_factor = p_factor; push_config(); }

	float get_volume_db() const { return volume_db; }
	void set_volume_db(float p_db) { volume_db = p_db; push_config(); }

	float get_binaural_spatial_blend() const { return binaural_spatial_blend; }
	void set_binaural_spatial_blend(float p_blend) { binaural_spatial_blend = p_blend; push_config(); }

	godot::TypedArray<godot::AudioEffect> get_effect_stack() const { return effect_stack; }
	void set_effect_stack(const godot::TypedArray<godot::AudioEffect> &p_stack);
};
